             ...
             (let ((line (read-line i 'concat)))
               ...
               (and
                ...
                (let* ((flush? (string-suffix? "<flush>\n" line))
                       (output (if flush? line
                                   (trace:format-trace line #:format "event")))
      ...
      (let loop ((paths (map list (node->transitions from lts))))
        (if (null? paths) '()
            (let ((paths (extend paths))
                  (shortest (find (compose (cute = to <>) transition-to car)
                                  paths)))
              (or shortest
                  (loop (reduce paths))))))))
...
transitions in different colors)."
  (let ((explored
    ...
    (cond
     ((equal? "aut" format) (lts->aut explored from (current-output-port)))
     ((equal? "dot" format)
      (let ((expect "black")
            (missing "magenta")
            (extraneous "cyan")
