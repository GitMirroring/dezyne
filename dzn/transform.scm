     ...
     (let* ((function (.function o))
       ...
       (if (.recursive? function) o
           (let* ((formals (ast:formal* function))
             ...
             (inline-functions statement)))))
    (($ <assign>)
     (let ((expression (.expression o)))
       (if (not (is-a? expression <call>)) o
           (let* ((statement (inline-functions expression))
                  (statement (substitute-return o statement)))
             (inline-functions statement)))))
    (($ <if>)
     (clone o #:then (inline-functions (.then o))
            #:else (and=> (.else o) inline-functions)))
    (($ <on>)
     (clone o #:statement (inline-functions (.statement o))))
    (($ <guard>)
     (clone o #:statement (inline-functions (.statement o))))
    (($ <compound>)
            ...
            (statements
             (let loop ((statements statements))
               (match statements
                 (((and ($ <variable>) v) t ...)
                  (let ((expression (.expression v)))
                    (if (not (is-a? expression <call>)) (cons v (loop t))
                        (let* ((values (ast:values (ast:type v)))
                               (default (car values))
                               (variable (clone v #:expression default))
                               (name (.name v))
                 ...
                 (()
                  '())))))
       (clone o #:elements (map inline-functions statements))))
    (($ <behavior>)
     (let* ((statement (inline-functions (.statement o)))
       ...
       (clone o #:statement statement
              #:functions functions)))
    (($ <component>)
     (clone o #:behavior (inline-functions (.behavior o))))
    (($ <interface>)
     (clone o #:behavior (inline-functions (.behavior o))))
    (($ <system>)
     o)
