...
(define (trace->string-trail-unmemoized trace)
...
(define trace->string-trail
  (let ((cache (make-weak-key-hash-table 523))
        (gc-buffer (make-gc-buffer 256)))
    (lambda (trace)
      (let ((key (pointer-address (scm->pointer trace))))
        (gc-buffer key)
        (or (and (not (is-a? (%sut) <runtime:port>)) (hash-ref cache key))
            (let ((result (trace->string-trail-unmemoized trace)))
