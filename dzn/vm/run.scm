  ...
  (let ((modeling-names (modeling-names port)))
    (if (null? modeling-names) '()
        (let* ((previous (.previous pc))
               ...
               (trail (.trail pc))
               (pc (clone pc #:trigger #f #:previous #f #:instance #f #:trail '() #:statement #f))
          ...
          (map (compose (cut clone <> #:previous previous #:trigger trigger #:instance instance #:statement statement) car) traces)))))
