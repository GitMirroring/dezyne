...
(define int-regexp (make-regexp ".*'Int\\(([^)]*)\\)"))
  ...
  (define (event->prefix event) ;; XXX vouw in trigger?, action?, reply?
    (or (and=> (regexp-exec in-regexp event)
        ...
        (and=> (regexp-exec bool-regexp event)
               (cute match:substring <> 1))
        (and=> (regexp-exec int-regexp event)
               (cute match:substring <> 1))
      ...
      (and (string? o)
           (or
            ...
            (and=> (regexp-exec state-regexp2 o)
                   (cute match:substring <> 1)))))
    (define (trim-tick o)
      (if (not (string-suffix? "'" o)) o
          ((@ (dzn code language makreel) makreel:name) o)))
      ...
      (let* ((label (edge-label edge))
        ...
        (and (pair? variables)
             ...
             (let* ((from (edge-from edge))
                    ...
                    (values (map (compose trim-tick string-trim) values))
                    (values (map (cute make <shared-value> #:value <>) values))
...
(define (code:normalize+determinism ast)
  ...
  (parameterize ((%normalize:short-circuit? code:short-circuit?))
    (let ((root ((compose
                  (with-root add-determinism-temporaries)
                  code:normalize-)
                 ast)))
      (when (> (dzn:debugity) 1)
        (ast:pretty-print root (current-error-port)))
