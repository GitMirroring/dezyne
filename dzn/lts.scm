...
(define (clone-edge e)
  (make-edge (edge-from e) (edge-label e) (edge-to e) #:tau? (edge-tau? e)))
    ...
    (define (detect-loops state entry-edge)
      ...
      (let ((node (vector-ref lts state)))
        (cond ((= (node-color node) %grey)
               (let* ((edge (clone-edge entry-edge))
...
(define (deadlock-nodes lts)
  ...
  (let* ((lts (remove-state-loops lts)))
    (define (exclude-edge? edge)
      (let ((label (edge-label edge)))
        (cond ((eq? label %<declarative-illegal>)
               #t)
              ((not (or (edge-tau? edge)
                        (string-prefix? "<external>." label)))
               #f)
              (else
               (let* ((to (vector-ref lts (edge-to edge)))
                      (edges (node-edges to)))
                 (and (not (node-cycle to))
                      (find exclude-edge? edges)))))))
    (define (exclude-node? node)
      (let ((edges (node-edges node)))
        (find exclude-edge? edges)))
    (define (edges? node)
      (let ((edges (node-edges node)))
        (find (negate exclude-edge?)
              (filter
               (compose not exclude-node? (cute vector-ref lts <>) edge-to)
               edges))))
    (filter (compose not edges? (cute vector-ref lts <>))
            (filter (compose (negate exclude-node?) (cute vector-ref lts <>))
