...
(define* (compose-parallel lts0 lts1)
  (let* ((size0 (vector-length lts0))
      ...
      (let* ((node0 (vector-ref lts0 state0))
        ...
        (define (step0 edge)
          (let ((label (edge-label edge))
                (to0 (edge-to edge)))
            (if (not (common? edge))
                (step node edge to0 state1)
                (let ((matches (filter (lambda (e) (equal? label (edge-label e)))
                                       (node-edges node1))))
                  (for-each (lambda (e) (step node edge to0 (edge-to e))) matches)))))
        (define (step1 edge)
          (let ((label (edge-label edge))
                (to1 (edge-to edge)))
            (if (not (common? edge))
                (step node edge state0 to1)
                (unless do-step0 ;; common steps already done in step0
                  (let ((matches (filter (lambda (e) (equal? label (edge-label e)))
                                         (node-edges node0))))
                    (for-each (lambda (e) (step node edge (edge-to e) to1)) matches))))))
    ...
    (if (zero? size0) lts1
        (begin
          (step #f #f (initial lts0) (initial lts1))
