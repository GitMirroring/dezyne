...
(define (annotate-exclude lts excludes)
  (define (exclude-label i node)
    (let* ((labels (map edge-label (node-succ node)))
           (label (find (cute memq <> excludes) labels)))
      (set-node-exclude?! node label)))
  (vector-for-each exclude-label lts))

(define (remove-illegal lts)
  (define (to-exclude? edge)
    (node-exclude? (vector-ref lts (edge-to edge))))
  (define (exclude i node)
    (let* ((new-node (clone-node node))
           (succ (filter (negate to-exclude?) (node-succ node))))
