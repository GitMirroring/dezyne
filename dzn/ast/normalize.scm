...
(define (purge-data o)
  ...
  (match o
    ...
    (($ <reply>)
     (cond ((or (not (.expression o))
                (is-a? (ast:type o) <extern>))
            (or (and=> (or (as (.expression o) <action>)
                           (as (.expression o) <call>))
                       purge-data)
                (clone o #:expression (make <literal>))))
           (else
            o)))
    (($ <return>)
     (cond ((or (not (.expression o))
    ...
    (($ <assign>)
     (let* ((variable (.variable o))
       ...
       (or (and type (not (is-a? type <extern>))
                (clone o #:expression (purge-data (.expression o))))
           (and=> (or (as (.expression o) <action>)
                      (as (.expression o) <call>))
                  purge-data)
           (let ((p (.parent o)))
             (and (not (or (is-a? p <compound>)
                           (is-a? p <variables>)))
