    ...
    (append wfce
            (cond ((not function)
                  ...
                  ((not (ast:eq? last-statement-block o))
                   `(,(wfc-error ((compose car wfc:continuation) o)
  ...
  (let* ((function (ast:parent o <function>))
    ...
    (if non-recursive? '()
        (let* ((assign (or (ast:parent o <assign>)
               ...
               (continuation ((compose car wfc:continuation) o))
               (continuation (if (or (ast:parent o <assign>)
                                     (ast:parent o <reply>)
                                     (ast:parent o <return>)
                                     (ast:parent o <variable>))
                                 ((compose car (@@ (dzn code makreel) makreel:continuation))
                                  continuation)
                                 continuation))
               (statement (.statement function))
               (continuation (and continuation
                                  (not (ast:eq? continuation statement))
          ...
          (cond
           ...
           ((is-a? continuation <return>)
            (let* ((continuation ((compose car (@@ (dzn code makreel) makreel:continuation))
                                  continuation))
                   (continuation
                    (and (not (ast:eq? continuation statement))
                         continuation)))
              (if (not continuation) '()
