              ...
              (match o
                (($ <interface>)
                 (let* ((in-types (map ast:type (ast:in-event* o)))
                        ...
                        (reply-enums (delete-duplicates reply-enums eq?))
  ...
  (let ((cache '()))
    (lambda (o)
      (or
       ...
       (let* ((calls (tree-collect (is? <call>) (.behavior o)))
              (calls functions (makreel:direct->direct+indirect calls))
              (functions (lset-union eq? functions (ast:function* (.behavior o))))
    ...
    (lambda (o)
      (or
       ...
       (let* ((returns (makreel:call-continuation* o))
              (returns (delete-duplicates returns eq?))
              (result
        ...
        `(,@(map (lambda (port)
                   (sm:sequence*
                    ...
                    (sm:goto (name (statement->process-name o))
                             (arguments (makreel:process-parens o)))))
                 (lset-difference eq? ports (map .port shared)))
          ,@(map (lambda (port)
                   (let ((shared (filter (compose (cute eq? port <>) .port)
                                         shared)))
                     (sm:sequence*
                      `(,(%flush-action port)
                                    ...
                                    (lambda (v)
                                      (let* ((p (.port v))
                                        ...
                                        (simple-format
                                         ...
                                         (port-prefix "s" p))))
                                    shared)))))))
                 (lset-intersection eq? ports (map .port shared)))
          ,@defers
          ,(sm:goto (name (statement->process-name next))
       ...
       (sm:sequence*
        `(,(ast->action trigger)
          ,(if (ast:in? trigger)
               ...
               (if (not (find (compose (cute eq? port <>) .port)
                              (%shared)))
                   (sm:goto (name (statement->process-name next))
                            (arguments (makreel:process-parens next)))
                   (sm:sequence*
               ...
               (sm:sequence*
                `(,(if (not (is-a? action-type <subint>)) reply
                  ...
                  ,@(if (or (ast:external? port)
                            (not (find (compose (cute eq? port <>) .port)
                                       (%shared)))) '()
                                       `(,(sum-state-action port))))))))))
          ...
          ,@(if (makreel:switch-context? o)
                ...
                `(,@(if (not void-reply?) '()
                  ...
                  ,(cond ((and=> port ast:external?)
                          (sm:goto (name (statement->process-name next))
                                   (arguments (makreel:process-parens next))))
                         ((find (compose (cute eq? port <>) .port)
                                (%shared))
                          (sm:sequence*
                           (sum-state-action port)
                           (sm:goto
...
(define-method (ast->process (model <component>) (o <action-reply>) (next <ast>))
  (let* ((port (.port o))
         (shared (%shared))
         (requires (filter (negate (cute eq? port <>))
...
(define-method (ast->process (model <component>) (o <the-end>) (next <ast>))
  (let ((provides-shared (filter (compose ast:provides? .port) (%shared))))
    (sm:process
      (name (statement->process-name o))
      (formals (makreel:process-formals o))
      (statement
       (sm:sequence*
        `(,@(if (not (makreel:defer-skip? model)) '()
          ...
          ,(sm:union*
            ...
            (sm:sequence*
             %queue-empty-action
             (if (null? provides-shared) (sm:goto
                                          (name (statement->process-name next))
                                          (arguments (makreel:process-parens
                                                      next)))
                 (sm:union*
                  `(,(sm:goto
                      (name (statement->process-name next))
                      (arguments (makreel:process-parens
                                  next)))
                    ,@(map (lambda (shared-state)
                             (let* ((port (.port shared-state))
                                    (interface (.type port)))
                               (sm:sequence*
                                (sum-state-action port)
                                (sm:goto (name (statement->process-name next))
                                         (arguments
                                          (map (lambda (shared-state)
                                                 (simple-format
                                                  #f "~a=~a (~a)"
                                                  (makreel:full-name shared-state)
                                                  (model-prefix (ast:name shared-state)
                                                                interface)
                                                  (port-prefix "s" port)))
                                               (filter (compose (cute eq? port <>) .port)
                                                       provides-shared)))))))
                           (delete-duplicates provides-shared
                                              (lambda (a b)
                                                (eq? (.port a) (.port b))))))))))))))))
...
(define-method (interface->scmackerel-unmemoized (o <interface>))
  (let* ((model-name (ast:dotted-name o))
         ...
         (calls functions (makreel:direct->direct+indirect calls))
         (functions (lset-union eq? functions (ast:function* behavior)))
         (functions (filter (negate (is? <expression-function>)) functions))
  ...
  (parameterize ((%model (.type o))
    ...
    (let* ((interface (.type o))
           ...
           (rename (sm:process
                     ...
                     (statement
                      (cond ((find (compose (cute eq? <> o) .port)
                                   (%shared))
                             proc)
                            (else
                             (sm:block (process
...
;;; XXX TODO: when/how to print a type's entities in a formal?
                  ...
                  (list
                   ...
                   (sm:comm-event (from
                                   (sm:multi-event (events
                                                    (list (%end-action p))))))))
                provides))))))
         (provides-interfaces (delete-duplicates (map .type provides) eq?))
         (provides-allow
          (sm:process
            (name "provides_allow")
            (statement
         ...
         (interfaces (delete-duplicates (map .type ports) eq?))
         (provides
          (sm:process
            (name "provides")
            (statement
...
(define-method (component-defer-processes (o <component>))
  (let* ((ports (ast:port* o))
         ...
         (shared (filter (negate (compose (cute member <> ports eq?) .port))
                         (%shared)))
         (component-behavior
          (sm:process
...
(define-method (semantics-main (o <component>))
  (let* ((provides (ast:provides-port* o))
         ...
         (provides-shared (lset-intersection eq? provides shared)))
      ...
      (statement
       (sm:union*
        `(,(sm:if* "blocking == {} && released == []"
                   (sm:sequence*
                    `(,@(map sum-state-action provides-shared)
                      ,(sm:union*
                        (sm:goto (name "semantics_main"))
                        (defer-qout-actions-async o name)))))
          ,(sm:sequence*
...
(define-method (semantics-async (o <component>))
  (let* ((provides (ast:provides-port* o))
         (requires (ast:requires-port* o))
         (shared (map .port (%shared)))
         (provides-unshared provides-shared
                            (lset-diff+intersection eq? provides shared)))
    ...
    (sm:process
      ...
      (formals `(,@%blocking-formals
                 "port: provides_ports"))
      (statement
       (let ((continuation (sm:union*
                            `(,(sm:if* (sm:invoke %no-port-predicate "port")
                                       (sm:union* (sm:goto (name "semantics_main"))
                                                  (sm:sequence*
                                                   (defer-qout-actions o name))))
                              ,@(map (cute provides-flush <> name) provides-unshared)
                              ,@(map (cute provides-shared-flush <> name) provides-shared)))))
         (sm:union*
          `(,(sm:sequence*
              (%end-action o)
              (sm:union*
               (sm:sequence*
                `(,%queue-empty-action
                  ,@(if (null? provides-shared) `(,continuation)
                        (map (lambda (shared-state)
                               (sm:sequence*
                                (sum-state-action shared-state)
                                continuation))
                             provides-shared))))
               (sm:sequence* %queue-not-empty-action
                             (sm:goto (name name) (arguments '())))))
...
(define-method (component-semantics-processes (o <component>))
  (let* ((provides (ast:provides-port* o))
         (requires (ast:requires-port* o))
         (shared (map .port (%shared)))
         (shared-requires (lset-intersection eq? requires shared))
