            ...
            `(,@(if (not (makreel:switch-context? o)) '()
              ...
              ,@(if (not void-reply?)
                    ...
                    (let* ((parent (tree:parent o))
                           (port-name (.name port))
                           (return (make <return> #:port.name port-name))
                           (return (graft parent return)))
                      `(,(ast->action return)
                        ,@(if (not (find (compose (cute eq? port <>) .port)
                                         (ast:shared* model))) '()
                                         `(,(sum-state-action port))))))
...
(define-method (semantics-main (o <component>))
  (let* ((provides (ast:provides-port* o))
         ...
         (shared-provides (lset-intersection eq? provides shared)))
      ...
      (let ((interface (.type p)))
        (sm:sum (type (%actions interface))
                (statement
                 (sm:sequence*
                  (sm:invoke (%out-action p) #:keep-constructor? #t)
                  (if (not (find (compose (cute eq? p <>) .port)
                                      (ast:shared* o)))
                      (sm:goto (name name)
                               (arguments
                                (list
