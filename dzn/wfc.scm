...
(define-method (modeling-silent (o <on>))
  (if (and (or (eq? (.silent? o) *unspecified*)
               (is-a? (.silent? o) <ast>)) ((@@ (dzn peg) any-modeling?) o))
  ...
  (let ((function (parent o <function>)))
    (if (or (not function)
            ...
            (let* ((continuation ((compose car wfc:continuation) o))
                   (continuation (if (is-a? continuation <variable>) ((compose car (@@ (dzn makreel) makreel:continuation)) continuation) continuation))
