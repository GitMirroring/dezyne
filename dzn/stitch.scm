...
(define* (bu:stitch root ports instances bindings #:key verbose?)
  ...
  (define* (stitch-instance instance blob #:key todo)
    (let* ((instance-todo (assq-ref todo instance))
           (todo (acons instance (1- instance-todo)
                        (alist-delete instance todo eq?))))
      (if (> instance-todo 1) (values blob todo)
          (let* ((requires (requires-bindings instance))
            ...
            (bu:stitch blob provides #:todo todo)))))
  (define* (bu:stitch blob instances #:key todo)
  ...
  (let* ((bottom-bindings (filter (compose not .instance .right) bindings))
         ...
         (blob (bu:stitch #() bottom-instances #:todo todo)))
...
(define-method (bu:model->lts root (system <system>) verbose?)
  (let* ((instances (ast:instance* system))
         ...
         (stitch? (and (pair? ports)
                       (pair? instances))))
    (if (not stitch?) #()
        (bu:stitch root ports instances bindings #:verbose? verbose?))))

;;(define-method (model->lts root (system <system>) verbose?)
;;  (bu:model->lts root system verbose?))
