...
(define (main args)
  ...
  (let* ((options (parse-opts args))
    ...
    (let* ((input (file-name->text file-name))
           ...
           (parse-tree (cdr (car alist)))
           (offset (or (and+pred=> (option-ref options 'offset #f)
                                   string->number)
                       (and+pred=>
                        (or (option-ref options 'line #f)
                       ...
                       parse-tree))
           (context (complete:context parse-tree offset)))
      (define (file-name->parse-tree file-name)
        (assoc-ref alist (resolve-file file-name)))
      (when (> debugity 2)
        (display "input:\n" (current-error-port))
