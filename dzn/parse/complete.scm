  ...
  (match o
    ...
    ((? (is? 'port))
     (cond ((and (not (slot o 'provides))
           ...
           (else
            (let* ((qualifiers (pke 'qual (tree:port-qualifier* o))))
              (append (if (pair? qualifiers) '() '("blocking" "external" "injected"))
                      (complete:interface-names (context:parent context 'component)))))))
    ((? (is? 'types-and-events))
    ...
    ((? (is? 'unknown-identifier))
     (complete:root (parent context tree?)
                    (context:parent context tree?) offset #:debug? debug?))
    ((and (? (is? 'or)) (? incomplete?)) (=> failure)
     (let* ((context (or (context:parent context 'compound)
                         (context:parent context 'on)))
            (triggers (and=> (is-a? (.tree context) 'on)
                             tree:trigger*)))
       (cond
        ((is-a? context 'compound)
         => (cute complete:statements <> offset))
        ((and (pair? triggers)
              (every complete? triggers)
              context)
         => (cute complete:statements <> offset))
        ((is-a? (.tree context) 'on)
         (complete:trigger-names context))
        (else
         (failure)))))
    (_
