...
(define (.type context)
  (let ((tree (.tree context)))
    (match tree
      ((? (is? 'assign)) (and=> (.reference tree) (compose .type (cute tree->context <> context))))
      ...
      ((? (is? 'reference)) (and=> (.variable context) .type))
      ((? (is? 'instance)) (and=> (.type-name tree) (cute context:lookup <> context))))))

(define (.variable context)
  (let ((tree (.tree context)))
    (match tree
      ((? (is? 'reference)) (and=> (.name tree) (cute context:lookup <> context))))))
...
(define (resolve-reference o name context)
  (assert-type o 'assign 'reference 'trigger-reference)
  (tree:lookup-reference name context))
...
(define (resolve-field-test o name context)
  ...
  (let* ((reference (.reference o))
         ...
         (variable (tree:lookup reference-name context))
   ...
   (and (or (is-a? name 'name)
        ...
        (cond
         ((parent context 'reference)
          => (cute resolve-reference <> name context))
         ...
         ((parent context 'trigger-reference)
          => (cute resolve-reference <> name context))
