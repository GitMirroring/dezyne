...
(define %time
  `(((dzn script) run-command)
    ...
    ((dzn parse) parse:merge-namespaces)
...
(define %measure
  `(((dzn parse) parse:file->string)
    ...
    ((dzn tree context) tree:memoize-context (,<tree:root>))
    ((dzn tree context) tree:context (,<tree>))
    ((dzn tree context) tree:ancestor (,<tree> ,<class>))
