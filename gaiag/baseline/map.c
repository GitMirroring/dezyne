 ...
 */
unsigned int map_hash_int(map* m, char* keystring){
 ...
 */
int map_put(map* m, char* key, void* value){
	int index;

	/* Find a place to put our value */
	index = map_hash(m, key);
	while(index == MAP_FULL){
		if (map_rehash(m) == MAP_OMEM) {
			return MAP_OMEM;
		}
		index = map_hash(m, key);
	}
 ...
 */
int map_get(map* m, char* key, void* *arg){
	int curr;
	int i;
 ...
 */
int map_iterate(map* m, map_f f, void* item) {
	int i;
