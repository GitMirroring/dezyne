...
(define-method (initialize (o <Reply3>) args)
  (next-method)
  (set! (.i o)
    (make <interface:I>
...
(define-method (i-done (o <Reply3>))
  (stderr "Reply3.i.done\n")
    (cond 
    (#t
      ...
      (cond ((equal? s '(Status Ok)) 
        (reply_fun o))
      (else 
        (reply_fun_arg o '(Status No)))))))
    (.reply-I-Status o))

(define-method (reply_fun (o <Reply3>) )
  (call/cc
   (lambda (return) 
    (set! (.reply-I-Status o) '(Status Yes)))))

(define-method (reply_fun_arg (o <Reply3>) s)
  (call/cc
   (lambda (return) 
