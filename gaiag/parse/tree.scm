...
(define (.type-name o)
  (assert-type o 'enum-literal 'port 'variable)
  (match o
...
(define (tree:name o) ;;; FIXME, see tree:name
  (match o
    ((? (is? 'event-name)) (slot o 'name))
    ((? (is? 'port)) (slot o 'name))
    ((? (is? 'scope)) (slot o 'name))
    ((? (is? 'var)) (slot o 'name))
    ((? (is? 'variable)) (slot o 'name))

    ((? (is? 'component)) (slot o 'compound-name))
    ((? (is? 'enum)) (slot o 'compound-name))
    ((? (is? 'int)) (slot o 'compound-name))
    ((? (is? 'instance)) (slot o 'name))
    ((? (is? 'interface)) (slot o 'compound-name))
    ((? (is? 'scope)) (slot o 'name))

    ((? (is? 'event)) (tree:name (slot o 'event-name)))

    ((? (is? 'namespace)) (slot o 'compound-name))))
...
(define (tree:statement* o)
  (assert-type o 'behaviour-compound 'blocking 'compound 'guard 'function 'if 'on)
  (slots o tree:statement?))
