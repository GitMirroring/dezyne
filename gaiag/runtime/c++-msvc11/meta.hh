...
namespace dezyne
{
  ...
  struct meta
  {
    ...
    meta(std::string&& name, std::string&& type, const component* address, const component* parent, std::vector<const component*>&& children, std::vector<std::function<void()>>&& ports_connected)
    : name(name)
    , type(type)
    , address(address)
    , parent(parent)
