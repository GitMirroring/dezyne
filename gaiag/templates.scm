    ...
    (syntax-case x ()
      ((_ name class language)
       (let* ((tname (datum->syntax x (symbol-append 't: (syntax->datum #'name))))
              ...
              (template (symbol->string (symbol-append 'gaiag/templates/ (syntax->datum #'language) '/ (syntax->datum #'name) '@ class-name)))
...
(define-syntax define-templates-base
  (lambda (x)
    (define (body language name xname tname func sep)
      (let ((dir (string-append "gaiag/templates/" (symbol->string (syntax->datum language)) "/")))
