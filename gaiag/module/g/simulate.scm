...
(define ((valued-action? port?) src)
  (match src
    (('variable name type ('action _ ___)) #t)
    (('assign identifier ('action _ ___)) #t)
    (_ #f)))
...
(define (eval-function-expression model ast state event trace expression)
  ...
  (let* ((members (om:member-names model))
    ...
    (match expression
      (('call function ('arguments arguments ___))
       ...
       (match ast
         ...
         (('guard expression statement)
          ...
          (if (eval-expression model state expression)
              (process model statement state event (cons ast trace))
              (values state #f #f #f trace)))
         (('illegal _ ___) (values state #f #f #f (cons ast trace)))
         (('action trigger ___)
          (stderr "****action: ~a\n" (->string trigger))
...
(define (->string src)
  (match src
    ...
    (('call function ('arguments arguments ___)) (->string (list function "("  (comma-join arguments) ")" )))
