...
(define (ast-> ast)
  (let* ((norm (norm-state (if (member (ast:name (ast:component ast))
                                     '(mangle argument2))
                             (ast:mangle ast)
                              ast))))
    (ast:register norm #t)
    (module-define! (resolve-module '(g csp)) 'ast norm)  ;; FIXME
