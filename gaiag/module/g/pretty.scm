...
(define (->string src)
  ...
  (match src
    ...
    (('arguments) "")
    (('arguments arguments ___) (->string (list "(" (comma-join (map ->string arguments)) ")")))
    (('parameters parameters ___) (->string (list "(" (comma-join (map ->string parameters)) ")")))
    (('parameter name type (or #f 'in)) (->string (list type " " name)))
    (('parameter name type dir) (->string (list dir " " type " " name)))
    (('parameter name type) (->string (list type " " name)))    
    ...
    (('type name ___) (->string name))
    (('otherwise value ___) (->string value))    
    (('triggers triggers ___) (comma-space-join (map ->string triggers)))

    ;; FIXME: c&p from csp.scm (and...TODO: c++.scm) grmbl
    (('group expression) (->string (list "(" (->string expression) ")")))
    (('expression expression ___) (expression->string expression))
    (('var identifier ___) (->string identifier))
    (('data data ___) (->string (list "$" data "$")))
    ...
    (('! ('expression expression ___)) (->string (list "!" (paren expression))))
