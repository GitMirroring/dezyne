  ...
  (if (or (and (pair? scope) (eq? (car scope) '*))) o
      (match o
        (('interface name types events behaviour)
         (rsp o `(interface ,((add-scope scope) name) ,((add-scope scope) types) ,((add-scope scope) events) ,((add-scope scope) behaviour))))
        ...
        (('behaviour name types variables functions statement)
         `(behaviour ,name ,((add-scope scope) types) ,((add-scope scope) variables) ,((add-scope scope) functions) ,((add-scope scope) statement)))
        (('name '* scope ...) o)
        (('name name) o)
        (('name name ...) (append scope name))
        (('events events ...) (cons 'events (map (add-scope scope) events)))
        (('formals formals ...) (cons 'formals (map (add-scope scope) formals)))
        (('functions functions ...) (cons 'functions (map (add-scope scope) functions)))
        ...
        (('event name signature direction)
         `(event ,name ,((add-scope scope) signature) ,direction))
        (('formal name type)
         `(formal ,name ,((add-scope scope) type)))
        (('formal name type direction)
         `(formal ,name ,((add-scope scope) type) ,direction))
        (('signature type)
         `(signature ,((add-scope scope) type)))
        (('signature type formals)
         `(signature ,((add-scope scope) type) ,((add-scope scope) formals)))
        (('function name signature recursive? statement)
         `(function ,name ,((add-scope scope) signature) ,recursive? ,statement))
        (('port name type direction injected)
         `(port ,name ,((add-scope scope) type) ,direction ,injected))
        (('variable name type expression)
         `(variable ,name ,((add-scope scope) type) ,((add-scope scope) expression)))
        (('expression ('name s ... name field))
         `(expression ,((add-scope scope) (cadr o))))
        (('type ('name s ... name))
