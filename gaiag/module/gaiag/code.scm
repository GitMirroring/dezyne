  ...
  (let* ((signature (.signature event))
    ...
    (or (and-let*
         (((is-a? model <component>))
          ...
          (trigger (find-trigger port event))
          (ons (filter trigger ons))
          (arguments (if (pair? ons)
                         (map (compose .name .value) ((compose .elements .arguments car .elements .triggers car) ons))
                         (map .name parameters)))
