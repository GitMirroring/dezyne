    ...
    (match src
      ...
      (($ <on> triggers statement)
       (or (and-let* ((trigger (find-trigger src port event)))
                     (->code model statement locals indent compound?))
           '$empty-statement$))
...
(define ((define-on model port snippet) event)
  (let* ((signature (.signature event))
         ...
         (comma (if (pair? parameters) "," ""))
         (comma-space (if (pair? parameters) "," ""))
    ...
    (or (and-let*
         (((is-a? model <component>))
          ...
          (ons (filter (find-trigger port event) ((gom:statements-of-type 'on) statement)))
          (deep-ons (filter (find-trigger port event) ((gom:collect <on>) statement)))
          (deep-ons (filter (lambda (o)
                              (and-let* ((t (find-trigger o port event))
                                         ((>0 (length 
                                               ((compose .elements .arguments) t)))))
                                        (not (is-illegal? (.statement o)))))
                            deep-ons))
          (arguments (if (pair? deep-ons)
                         (map (compose .name .value) ((compose .elements .arguments car .elements .triggers car) deep-ons))
                         (map .name parameters)))
          (parameters (map (lambda (p a) (make <gom:parameter> :name a :type (.type p) :direction (.direction p))) parameters arguments))
          (parameter-list (map (lambda (x) (code:->code model x)) parameters))
          ...
          (locals (let loop ((parameters parameters) (locals '()))
                    (if (null? parameters)
                        locals
                        (loop (cdr parameters)
          ...
          (parameters ((join) parameter-list))
          (statement        
         ...
         (animate snippet `((port ,(.name port))
                            ...
                            (statement ,statement)
                            (type ,(.name type)))))
        (let* ((parameter-types (map (lambda (parameter)
                                       (animate-snippet 'parameter-type `((type ,(->code model (.type parameter))) (out? ,(member (.direction parameter) '(inout out))))))
                                     parameters))
               (parameter-list (map (lambda (x) (code:->code model x)) parameters))
               
               (arguments (map .name parameters))
               (number (number->string (length arguments)))
