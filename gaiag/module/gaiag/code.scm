...
(define* ((*ns-scope* model :optional (infix (string->symbol "::"))) o)
  (stderr "*ns-scope*: ~a\n" o)
  (if (and (>2 (length o)) (not (eq? (cadr (.name model)) (car o)))) ((->symbol-join infix) (cons null-symbol o))
...
(define* (->code- model src :optional (locals '()) (indent 1) (compound? #t))
  ...
  (let ((port (statements.port))
    ...
    (match src
      ...
      ;;                              (name ,(om:name src)))))
      ;; (($ <type> ('name scope ... name)) (=> failure)
      ;;  (if (not (equal? scope ((compose cdr .name) model))) (failure)
      ;;      (snippet 'type-local `((space ,space) (scope ,(.name model)) (name ,name)))))
      ;; ((and ($ <type> name ;; scope
      ;;          ) (? enum?))
      ;;  (snippet 'type-enum `((space ,space) (scope ,(om:scope src)) (name ,(om:name src)))))
      ;; (($ <type> name)
      ;;  (snippet 'type `((space ,space) (scope ,(om:scope src)) (name ,(om:name src)))))
      (($ <type> name)
       (snippet 'type `((space ,space) (scope ,((*ns-scope* model) (cdr name))) (name ,(om:name src)))))
...
(define ((declare-io model snippet) event)
  (let* ((name (.name event))
         ...
         (return-type (return-type model #f event))
...
(define ((define-on model port snippet) event)
  (let* ((signature (.signature event))
         ...
         (interface (.type port))
         (return-type (return-type model port event))
         (formals (.formals signature))
...
(define (return-type model port event)
