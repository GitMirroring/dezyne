                             ...
                             (let loop ((parameters ((compose .elements .parameters .signature) event))
                               ...
                               (if (null? arguments)
                                   ...
                                   (let* ((parameter (car parameters))
                                          (type (->code model (.type parameter) '(foo bar baz)))
...
(define ((declare-io model snippet) event)
  (let* ((name (.name event))
         ...
         (parameter-objects (.elements parameters))
    ...
    (animate snippet `((name ,name) (comma ,comma) (comma-space ,comma-space) (parameters ,parameters) (parameter-list ,parameter-list) (parameter-objects ,parameter-objects) (parameter-types ,parameter-types) (return-type ,return-type)))))
...
(define ((define-on model port snippet) event)
  (let* ((signature (.signature event))
         ...
         (enum (if (eq? return-type 'void) #f (gom:enum (gom:interface port) type)))
         (parameters (.parameters signature))
         (argument-list (map .name (.elements parameters)))
         (number (number->string (length argument-list)))
         (arguments (comma-join argument-list))
         (locals (let loop ((parameters (.elements parameters)) (locals '()))
                   (if (null? parameters)
                       locals
                       (loop (cdr parameters)
