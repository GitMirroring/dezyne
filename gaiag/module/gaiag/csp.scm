...
(define* (csp-transform ast src :optional (inevitable-optional? #f) (channel #f) (provided-on? #t) (function? #f))
  (let* ((model (or (gom:interface ast) (gom:component ast)))
	 (model-name (.name model))
	 (behaviour (.name (.behaviour model)))
         (component? (is-a? model <component>))
         (contP (if function? "PF'" "P'")))
    (=>string ast
     (match src
       (($ <csp-on>)
        (let* ((triggers (.elements (.triggers src)))
               ...
               (transformed (if ig? #f (csp-transform ast statement inevitable-optional? channel provided-on? function?)))
               (transformed-end (csp-transform ast the-end inevitable-optional? channel provided-on? function?)))
          (list IG? channel "?x:{" event-names "}" " ->\n" transformed transformed-end)))
       (($ <expression>) src)
       (($ <reply> expression)
        (let ((expression (csp-transform ast expression inevitable-optional? channel provided-on? function?)))
       ...
       (($ <function> name ($ <signature> type ($ <parameters> '())) recursive? statement)
        (let ((transformed (csp-transform ast statement inevitable-optional? channel provided-on? #t)))
          (list name " = \\ PF',V' @ context_func_(" transformed ")(PF',V')\n")))
       (($ <function> name signature recursive? statement)
        (let ((body (csp-transform ast statement inevitable-optional? channel provided-on? #t)))
          (list name " = \\ PF',V',F' @ context_func_args_(F',\n" body ")(PF',V')\n")))
       (($ <csp-call> identifier ($ <arguments> '()) context)
        (list "callvoid_(\\ P',V' @ " identifier "(" contP ",V'))"))
       (($ <csp-call> identifier arguments context)
        (let ((arguments (.elements arguments)))
          (list "callvoid_(\\ P',V' @ " identifier "(" contP ",V',(\\ (" context ") @ (" arguments "))(V')))")))
       (($ <csp-if>)
        (let ((context (.context src))
              (expression (csp-expression->string ast (.expression src)))
              (then (csp-transform ast (.then src) inevitable-optional? channel provided-on? function?))
              (else (csp-transform ast (.else src) inevitable-optional? channel provided-on? function?)))
          (list "\\ P',(" context ") @ ifthenelse_(" expression ",\n" then ",\n" else "\n)(P',(" context "))")))
       (('context-active (context var ($ <action> ($ <trigger> port event))) stat)
        (let ((stat (csp-transform ast stat inevitable-optional? channel provided-on? function?)))
       ...
       (('context-active (context var ($ <call> identifier ($ <arguments> '()))) stat)
        (let ((stat (csp-transform ast stat inevitable-optional? channel provided-on? function?)))
          (list "context_active_(\\ P',V' @ " identifier " (" contP ",V'),\n" stat ")")))

       (('context-active (context var ($ <call> identifier (and ($ <arguments>) (get! arguments)))) stat)
          (let ((stat (csp-transform ast stat inevitable-optional? channel provided-on? function?)))
          (list "context_active_(\\ P',V' @ " identifier " (" contP ",V',(\\ (" context ") @ (" (arguments) "))(V')),\n" stat ")")))
       ...
       (('assign-active (context var ($ <call> identifier (and ($ <arguments>) (get! arguments)))) expressions)
        (list "assign_active_(\\ P',V' @ " identifier " (" contP ",V',(\\ (" context ") @ (" (arguments) "))(V')),\n\\ ((" context "))," var " @ (" expressions "))"))
       (('context (context var expression) stat)
        (let ((stat (csp-transform ast stat inevitable-optional? channel provided-on? function?)))
          (list "context_(\\ (" context ") @ " expression ",\n" stat ")" )))
       (('semi stat1 stat2)
        (let ((first (csp-transform ast stat1 inevitable-optional? channel provided-on? function?))
              (second (csp-transform ast stat2 inevitable-optional? channel provided-on? function?)))
          (list "semi_(" first ",\n" second ")")))
       (('skip) "skip_")
       ('() "(\\ P',V' @ P'(V'))")
       ((? symbol?) src)
