...
(define (->string src)
  (match src
    ...
    (($ <arguments> '()) "")
    (($ <arguments> arguments) (->string (list "(" (comma-join (map (compose ->string .name .value) arguments)) ")")))
    ...
    (($ <signature> type ($ <parameters> '())) (->string (->string type)))
    (($ <signature> type parameters)
     (->string (list (->string type) (->string parameters))))
