          ...
          (match (.ast info)
            ...
            (($ <action> action)
             ...
             (let* ((q-info #f)
                    (infos
                     (cond
                      ;; ctrl.return fix in robot.
                      ;; FIXME: this case is to avoid h.dzn's ITransfer
                      ;; to bail out with an error, lacking the `tcalibrated'
                      ;; action upon the tcalibrate event
            ...
            (('compound) (list (clone info :ast #f)))
