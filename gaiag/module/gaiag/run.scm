...
(define (eval-function-expression model trigger info)
  (let* ((members (om:member-names model))
    ...
    (match expression
      ...
      (($ <action> a-trigger)
       (debug "INFO0: ~a\n" info)
       (let* ((info (next-value model info a-trigger))
              (reply (.reply info)))
         (if (is-a? model <interface>) (list ((cons-trace expression) info))
             (let ((info (clone <info> info :ast expression)))
               ((run model trigger) info)))))
                     ...
                     (i-infos (map
                               (lambda (i-info)
                                 (let* ((port (.port a-trigger))
                                        ...
                                        (i-trace (if (om:typed? interface a-trigger) (.trace i-info)
                                                     (cons return (cdr (.trace i-info)))))
    ...
    (map
     (lambda (i-info)
       (let ((i-info (set-state i-info port (.state i-info))))
         (clone <info> component-info :state-alist (.state-alist i-info) :trace (append trace (reverse (.trace i-info))) :error (.error i-info))))
     i-infos)))
  ...
  (let loop ((info component-info))
    ...
    (if (not (peeq info))
        ...
        (let* ((debug (stderr "flushing: ~a\n" (peeq info)))
               ...
               (port (.port trigger))
               (interface (run:import (.type (om:port model port))))
               (i-state (get-state component-info interface))
...
(define ((handle-return model trigger ast) info)
  (let* ((port (.port trigger))
    ...
    (if (modeling? interface trigger) info
        (let* ((foo (stderr "expecting-return[~a, ~a]:\n" (.name model) (->symbol trigger)))
