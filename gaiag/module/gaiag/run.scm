...
(define* (run-trigger model info :optional (reverse identity) (active? #f))
  (define (run-trigger-from-trail info)
    (let* ((info (next-trigger model info))
           ...
           (modeling? (and (is-a? model <interface>)
                           (let ((triggers (map .event (om:find-triggers model))))
                             (and (not (member (.event trigger) triggers))
                                  (stderr "NO SUCH TRIGGER[triggers: ~a], REQUIRED-OUT?: ~a\n" (map ->symbol triggers) (->symbol trigger)))))))
      (if (not modeling?)

          (begin (seen! model (.state info) (.ast info) trigger)
                 (stderr "NON-MODELING[~a]: ~a\n" (.name model) (->symbol trigger))
                 (run model trigger info))
          
          (let ((triggers (map (lambda (e) (make <trigger> :port (.port trigger) :event e)) '(inevitable optional))))
            (append-map (lambda (t) (run model t info)) triggers)))))

  (debug "run-trigger[~a ~a] " (.name model) (state->string (.state info)))
           ...
           (infos (if (or (is-a? model <component>)
                      ...
                      (let* ((triggers (map .event (om:find-triggers model)))
                             ...
                             (optional (if (not (member 'optional triggers)) '()
                                           (prune (run model (make <trigger> :port port :event 'optional) info))))
                             (modelling (append inevitable optional))
                             (infos (run-trigger-from-trail info)))
                        (append infos modelling)))))
      (map
       (lambda (info)
         (clone <info> info :trace (reverse (.trace info))))
       infos))))
    ...
    (match expression
      (($ <call> function ('arguments arguments ...))
       (let* ((f (om:function model function))
              ...
              (info (clone <info> info :ast statement :state state))
  ...
  (let* ((ast (.ast info))
    ...
    (let
     ...
     (match (.ast info)
                                     ...
                                     (lambda (i-info)
                                       (let ((i-info (set-state i-info port (.state i-info))))
                                         ...
                                         (clone <info> o-info :state-alist (.state-alist i-info)
                                                ;; :trace (append (cons ast trace) (reverse (.trace i-info)))
                                                :trace (append trace (reverse (.trace i-info)))
                                                :error (.error info))

                                         ))
                                     i-infos))
                             ;; continue with component on's STATEMENT
                             (infos (apply append
                                           (map (lambda (info)
                                                  (run model trigger info))
                                                infos)))
                                 ...
                                 (let* ((foo (debug "TYPED[~a]: ~a\n" trigger (om:typed? model trigger)))
                                        ...
                                        (trace (cond ((null? trace) (list trigger))
                                                     ...
                                                     ((and (is-a? model <interface>)
                                                           (not (.port trigger))
                                                           (member (.event trigger) '(inevitable optional)))
                                                      (append (list trigger ast) trace))
                                                     (else (append (list trigger ast) trace (list return)))))
                                        (x (debug "trace: ~a\n" trace)))
                                   ((set-trace trace) info)))
                               infos (iota (length infos))))))
              (stderr "result: infos: ~a\n" (length infos))
              infos)
            '()))
       ...
       (($ <action> a-trigger)
        (let* ((port (.port trigger))
               (a-port (.port a-trigger))
               (a-trigger (if (and port (not a-port)) (make <trigger> :port port :event (.event a-trigger)) a-trigger))
               ;;(ast (rsp ast (make <action> :trigger a-trigger)))
               (info+ast ((cons-trace ast) info))
               (o-info (clone <info> info)))
          (if (and (is-a? model <component>)
                   (stderr "CONSIDERING TRACE INTERFACE[~a]: ~a\n" (->symbol a-trigger) (not (eq? a-port (.name (om:port model)))))
                   (stderr "A-PORT: ~a\n" a-port)
                   (stderr "TRIGGER: ~a\n" (->symbol trigger))
                   (stderr "A-TRIGGER: ~a\n" (->symbol a-trigger))
                     ...
                     (i-infos (map
                               (lambda (i-info)
                                 (let* ((port (.port a-trigger))
                                        ...
                                        ;; Do not add PORT.RETURN for modeling event
                                        (return (if (or (member (.event a-trigger) '(inevitable optional))
                               ...
                               i-infos))
                     (i-infos (filter trace? i-infos)))
                (debug "FIXME: trigger???: ~a\n" trigger)
                i-infos)
              (let ((info info+ast))
                (if (is-a? model <component>)
                    (list (enq model info a-trigger))
                    (list (next-action model info a-trigger)))))))
              ...
              (let ((statement (car statements)))
                (let* ((state (if (is-a? statement <variable>)
                       ...
                       (frame (if (is-a? statement <variable>)
                                  (1+ frame) frame))
                       (info (clone <info> loop-info :ast statement :state state :trace '()))
                       (infos (run model trigger info))
                       (infos (prune infos))
...
(define (next-return model info trigger)
  ...
  (let* ((event (.event trigger)))
    (if (member event '(inevitable optional)) info
        (let* ((port (.port trigger)) ;; FIXME: better filter trail in component before we pass to interface?
               (info (if (is-a? model <component>) info
                         (skip-trail model info port)))
               (trail (.trail info))
          ...
          (if (null? trail)
              ...
              (let ((next (symbol->trigger (car trail))))
                (stderr "NEXT: ~a\n" next)
                (stderr "RETURN: ~a\n" return)
                (cond ((equal? next return) ;; FIXME: match in Component!?
