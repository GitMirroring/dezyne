           ...
           (modeling? (and (is-a? model <interface>)
                           ...
                           (let ((triggers (map .event (om:find-triggers model))))
                             (and (not (member (.event trigger) triggers))
                                  (stderr "NO SUCH TRIGGER, REQUIRED-OUT?: ~a\n" trigger))))))
      (if (not modeling?)
  ...
  (let* ((ast ((compose .statement .behaviour) model))
    ...
    (let* ((trigger (symbol->trigger (car (.trail info))))
           ...
           (infos (if (or (is-a? model <component>)
                      ...
                      (let* ((inevitable (prune (run model (make <trigger> :port port :event 'inevitable) info)))
                             (optional (prune (run model (make <trigger> :port port :event 'optional) info)))
