...
(define (process-event model event info)
  ...
  (map
   (lambda (info)
     (make <info> :trail (.trail info) :state (.state info) :reply (.reply info) :state-alist (.state-alist info) :trace (reverse (.trace info))))
    ...
    (match expression
      (($ <call> function ('arguments arguments ...))
       (let* ((f (om:function model function))
              ...
              (info (make <info> :trail (.trail info) :state (.state info) :ast (.ast info) :return #f :reply (.reply info) :state-alist (.state-alist info) :trace (list f)))
              (infos (process model event info))
              (info (if (and infos (=1 (length infos))) (car infos) (make <info>))))
         info))
      (($ <action> ($ <trigger> port event))
       (let ((return (eval-expression model state (next-value info event))))
         (make <info> :trail (.trail info) :ast ast :state state :reply reply :return return :state-alist (.state-alist info) :trace trace)))
      (_ (let ((return (eval-expression model state expression)))
           (make <info> :trail (.trail info) :ast ast :state (.state info) :reply (.reply info) :return return :state-alist (.state-alist info) :trace (.trace info)))))))
     ...
     (match ast
       (($ <on> ('triggers t ...) statement)
        (if (member event t equal?)
            (let* ((info (make <info> :trail trail :ast statement :state state :reply reply :return #f :state-alist (.state-alist info) :trace '()))
                   (infos (process model event info))
                   (info (if (and infos (=1 (length infos))) (car infos) (make <info>))))
              (if (pair? (.trace info))
                  (let ((return (next-return info (.port event))))
                    (list (make <info> :trail (.trail info) :ast (.ast info) :state (.state info) :reply (.reply info) :return #f :state-alist (.state-alist info) :trace (append (list (list 'return return)) (.trace info) (cons ast trace)))))
                  (list (make <info> :trail (.trail info) :ast #f :state (.state info) :reply (.reply info) :return #f :state-alist (.state-alist info) :trace '()))))
            (list (make <info> :trail trail :ast #f :state state :reply (.reply info) :return #f :state-alist (.state-alist info) :trace '()))))
       (($ <guard> expression statement)
        (if (eval-expression model state expression)
            (let* ((info (make <info> :trail trail :ast statement :state state :reply reply :return #f :state-alist (.state-alist info) :trace '()))
                   ...
                   (info (if (and infos (=1 (length infos))) (car infos) (make <info>))))
              (if (pair? (.trace info))
                  (list (make <info> :trail (.trail info) :ast (.ast info) :state (.state info) :reply (.reply info) :return #f :state-alist (.state-alist info) :trace (append (.trace info) (cons ast trace))))
                  (list (make <info> :trail (.trail info) :ast #f :state (.state info) :reply (.reply info) :return #f :state-alist (.state-alist info) :trace '()))))
            (list (make <info> :trail trail :ast #f :state state :reply reply :return #f :state-alist (.state-alist info) :trace '()))))
       (($ <illegal>)
        (list (make <info> :trail trail :ast #f :state state :reply reply :return #f :state-alist (.state-alist info) :trace (cons ast trace))))
       (($ <action> trigger)
        (let* ((trace (cons ast trace))
          ...
          (if port
              (let* ((interface (simulate:import (.type (om:port model port))))
                     ...
                     (i-info (make <info> :trail trail :ast i-ast :state i-state :reply reply :return #f :state-alist (.state-alist info) :trace '()))
                ...
                (list (make <info> :trail (.trail i-info) :ast #f :state state :reply reply :return #f :state-alist (.state-alist i-info) :trace (append (reverse (.trace i-info)) trace))))
              (list (make <info> :trail trail :ast #f :state state :reply reply :return #f :state-alist (.state-alist info) :trace trace)))))
       (($ <assign> identifier expression)
        (let* ((info (make <info> :trail trail :ast expression :state state :reply  reply :return #f :state-alist (.state-alist info) :trace (cons ast trace)))
               (info (eval-function-expression model event info)))
          ;; FIXME: doorgeefluik
          (list (make <info> :trail (.trail info) :ast #f :state (var! (.state info) identifier (.return info)) :reply (.reply info) :return #f :state-alist (.state-alist info) :trace (.trace info)))))
       (($ <call> function ('arguments arguments ...))
        (let* ((info (make <info> :trail trail :ast ast :state state :reply reply :return #f :state-alist (.state-alist info) :trace (cons ast trace)))
               (info (eval-function-expression model event info)))
          ;; FIXME: doorgeefluik
          (list (make <info> :trail (.trail info) :ast #f :state (.state info) :reply (.reply info) :return (.return info) :state-alist (.state-alist info) :trace (.trace info)))))
       ...
       (($ <if> expression then #f)
        (if (eval-expression model state expression)
            (process model event (make <info> :trail trail :ast then :state state :reply reply :return #f :state-alist (.state-alist info) :trace (cons ast trace)))
            (make <info> :trail trail :ast #f :state state :reply reply :return #f :state-alist (.state-alist info) :trace (cons ast trace))))
       (($ <if> expression then else)
        (if (eval-expression model state expression)
            (process model event (make <info> :trail trail :ast then :state state :reply reply :return #f :state-alist (.state-alist info) :trace (cons ast trace)))
            (process model event (make <info> :trail trail :ast else :state state :reply reply :return #f :state-alist (.state-alist info) :trace (cons ast trace)))))
       (('compound) (list (make <info> :trail trail :ast '() :state state :reply reply :return #f :state-alist (.state-alist info) :trace trace)))
       (#f (list (make <info> :trail trail :ast '() :state state :reply reply :return #f :state-alist (.state-alist info) :trace trace)))
                     ...
                     (trace (if (pair? loop-infos)
                                (if (=1 (length loop-infos))
                                    ...
                                    (and (stderr "traces:\n")
                                         ...
                                         (throw 'non-det "TODO")))
                               '())))
                (list (make <info> :trail (.trail loop-info) :ast '() :state (.state loop-info) :reply (.reply loop-info) :return #f :state-alist (.state-alist loop-info) :trace trace)))
              (let* ((statement (car statements))
                     (info (make <info> :trail trail :ast statement :state state :reply reply :return #f :state-alist (.state-alist info) :trace '()))
                     (infos (process model event info))
                     (info (if (and infos (=1 (length infos))) (car infos) (make <info>))))
                (if (pair? (.trace info))
       ...
       (('compound statements ...)
        (let loop ((statements statements) (loop-info (make <info> :trail trail :ast ast :state state :reply reply :return #f :state-alist (.state-alist info) :trace (cons ast trace))) (frame 0))
          ;;(stderr "LOOP: ~a\n" loop-info)
          (if (null? statements)
              (list (make <info> :trail (.trail loop-info) :ast '() :state (drop (.state loop-info) frame) :reply (.reply loop-info) :return #f :state-alist (.state-alist loop-info) :trace (.trace loop-info)))
              (let ((statement (car statements)))
                (let* ((state (if (is-a? statement <variable>)
                       ...
                       (frame (if (is-a? statement <variable>)
                                  (1+ frame) frame))
                       (info (make <info> :trail (.trail loop-info) :ast statement :state (.state loop-info) :reply (.reply loop-info) :return #f :state-alist (.state-alist loop-info) :trace '()))
                       (infos (process model event info))
                       (info (if (and infos (=1 (length infos))) (car infos) (make <info>)))
                       (info (make <info>
                               :trail (.trail info)
                               ...
                               :reply (.reply info)
                               :return #f
                               :state-alist (.state-alist info)
                               :trace (append (.trace info) (.trace loop-info)))))
                  (loop (cdr statements) info frame))))))
       (($ <return> expression)
        (let ((return (eval-expression model state expression)))
          (list (make <info> :trail trail :ast #f :state state :reply reply :return return :state-alist (.state-alist info) :trace (cons ast trace)))))
       (($ <reply> expression)
        (let ((return (eval-expression model state expression)))
          (set-return! info return)
          (list (make <info> :trail trail :ast ast :state state :reply reply :return #f :state-alist (.state-alist info) :trace (cons ast trace)))))))))
