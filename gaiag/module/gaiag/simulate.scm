    ...
    (match (.ast info)
                          ...
                          (lambda (info count)
                            (let* ((return (next-return info (.port event)))
                                   ...
                                   (trace (if (or (null? trace)
                                              ...
                                              trace
                                              (append (cons ast trace) (list (rsp ast (list 'return return)))))))
                              (clone <info> info :trace trace)))
                          infos (iota (length infos)))))
             (if (pair? infos)
                 infos
                 (list info)))
           (list (clone <info> info :ast #f :trace '()))))
      (($ <guard> expression statement)
       (if (eval-expression model state expression)
           ...
           (list (clone <info> info :ast #f :trace '()))))
      (($ <illegal>)
       (list (clone <info> info :ast #f :trace (cons ast trace))))
      (($ <action> trigger)
       (let* ((port (.port trigger))
              (info (clone <info> info :trace (cons ast trace))))
         (next-action info trigger)
         (let* ((trace (if (trace? info) (cons ast (.trace info)) '())))
           (list (clone <info> info :ast #f :trace trace)))))
      (($ <assign> identifier expression)
       (let* ((info (clone <info> info :ast expression :trace (cons ast trace)))
              (info (eval-function-expression model event info)))
         (list (clone <info> info :ast #f :state (var! (.state info) identifier (.return info))))))
      (($ <call> function ('arguments arguments ...))
