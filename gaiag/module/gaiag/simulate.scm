  ...
  (and state
       (match ast
         ...
         ((and ('compound statements ...) (? om:declarative?))
          (let loop ((statements statements) (loop-state state) (loop-return #f) (loop-trace  '()))
            (if (null? statements)
                (values loop-state '() #f loop-return
                        (if (pair? loop-trace)
                            (append loop-trace (cons ast trace))
                            '()))
         ...
         (('compound statements ...)
          (let loop ((statements statements) (loop-state state) (loop-return #f) (loop-trace (cons ast trace)) (frame 0))
            (if (null? statements)
                (values (drop loop-state frame) '() #f loop-return loop-trace)
                (let ((statement (car statements)))
