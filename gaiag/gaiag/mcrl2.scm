...
(define-method (mcrl2:reply-types (o <interface>))
  (let* ((events (om:events o))
         (voidevent (clone (car events) #:signature (make <signature>))))
    (delete-duplicates (cons voidevent events) (lambda (a b) (om:equal? ((compose .type .signature) a) ((compose .type .signature) b)))))
