...
(define-method (code:dump (o <foreign>))
  (let* ((dir (command-line:get 'output "."))
         (stdout? (equal? dir "-"))
         (dir (string-append dir "/" (dzn:dir o)))
         (name (symbol->string (code:skel-file o)))
         (iext (symbol->string (dzn:extension (make <interface>))))
         (cext (symbol->string (dzn:extension (make <component>)))))
    (when (code:header?)
      (if stdout? (begin (when (code:header?) ((%x:header) o)) ((%x:source) o))
          (begin (when (code:header?) (with-output-to-file (string-append dir name iext) (cut (%x:header) o)))
