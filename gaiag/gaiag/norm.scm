...
(define* ((expand-on #:optional (compare norm:on-equal?) model) o)
  (match o
    ...
    ((? (is? <component>)) (tree-map (expand-on compare o) o))
    ((? (is? <interface>)) (tree-map (expand-on compare o) o))
    (($ <functions>) o)
    ((and (? (is? <component>) (= .behaviour behaviour)))
    ...
    ((? (is? <component-model>)) o)
    (($ <skip>) o)
    ((? (is? <ast>)) (tree-map (expand-on compare model) o))
    (_ o)))
...
;; push all ons into first guard, discard the rest
  (match o
    ...
    ((? (is? <component-model>)) o)
    (($ <skip>) o)
    ((? (is? <ast>)) (tree-map aggregate-guard-g o))
    (_ o)))
...
(define (combine-guards o)
  (match o
    ...
    ((and (? (is? <interface>) (= .behaviour behaviour)))
     (clone o #:behaviour (combine-guards behaviour)))
    ((? (is? <component-model>)) o)
    ((? (is? <ast>)) (tree-map combine-guards o))
    (_ o)))
...
(define* ((passdown-blocking #:optional (blocking? #f)) o)
  ...
  (match o
    ...
    ((and ($ <blocking>) (= .statement (and ($ <on>) (= .statement statement))))
     (clone (.statement o) #:statement ((passdown-blocking #t) statement)))
    ...
    ((and (? (is? <interface>) (= .behaviour behaviour)))
     (clone o #:behaviour ((passdown-blocking blocking?) behaviour)))
    ((? (is? <component-model>)) o)
    ((? (is? <ast>)) (tree-map (passdown-blocking blocking?) o))
    (_ o)))
