...
(define* (animate-input module #:optional (file-name "<stdin>"))
  (catch #t
    ...
    (lambda (key . args)
      (let* ((alist (car args))
             (tell (or (and=> (assoc-ref alist 'tell) car)
                       (ftell (current-input-port))))
             (options (or (and=> (assoc-ref alist 'options) car)
                          '("function" "" '() #f)))
             (format-string (cadr options))
             (format-args (caddr options))
             (start (assoc-ref alist 'start))
             ...
             (catch #t
               ...
               (lambda (key . args)
                 (let* ((tell (cons
                        ...
                        (start (if (= (length tell) 1) (car tell)
                                   (reduce + 0 (append (cdr tell) %start))))
                        (options (car (or (assoc-ref (car args) 'options)
                                          (list args)))))
                   (throw key (append `((tell . ,tell)
                                        (start . ,start)
                                        (options ,options))))))))))))
