...
(eval-when (compile load expand)
  ...
  (define* (display-join proc o #:optional (grammar '("")))
    ...
    (define (body language name xname tname func sep)
      (let* ((o (datum->syntax x 'o))
             ...
             (grammar (if (not (syntax->datum sep)) '("")
                          (with-input-from-file (string-append dir (symbol->string (syntax->datum sep))) read)))
             (grammar (datum->syntax x grammar)))
        #`(begin
