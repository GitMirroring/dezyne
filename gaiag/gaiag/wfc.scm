...
(define-method (assign (o <ast>))
  (let* ((assign-type (ast:type o))
    ...
    (cond ((and (is-a? o <variable>) (is-a? expression-type <void>))
          ...
          ((and (is-a? expression-type <enum>)
                (is-a? expression <enum-literal>)
                (let ((fields (ast:field* expression-type))
                      (field (.field expression)))
                  (and (not (member field fields))
                       `(,(wfc-error o (format #f "no field `~a' in enum ~a; expected ~a"
                                               field
                                               (type-name assign-type)
                                               (string-join (map symbol->string fields) ", ")))
                         ,(wfc-error assign-type "enum declared here"))))))
          (else '()))))
...
(define-method (action (o <action>))
  (let ((event (.event o))
    ...
    (cond ((and (is-a? model <interface>) (ast:in? event))
...
(define-method (trigger (o <trigger>))
  (let ((event (.event o)))
    (if (not event) '()  ;: FIXME: handled elsewhere?
        (let ((model (parent o <model>)))
          (cond ((and (is-a? model <interface>) (ast:out? event))
...
(define-method (call-context (o <behaviour>))
  (append-map call-context (tree-collect (disjoin (is? <action>) (is? <call>)) o)))

(define-method (call-context (o <ast>))
  (let ((p (.parent o))
        (name (ast-name (class-of o))))
    (cond ((and (not (is-a? p <variable>))
                (or (is-a? p <expression>)
                    (and (is-a? p <if>)
                         (not (ast:eq? o (.then p)))
                         (not (ast:eq? o (.else p))))))
           `(,(wfc-error o (format #f "~a in expression" name))))
          ((and (not (parent o <on>))
                (not (parent o <function>)))
           `(,(wfc-error o (format #f "~a outside on" name))))
          ...
          ((and (not (is-a? (ast:type o) <void>))
                (not (is-a? p <assign>))
                (not (is-a? p <variable>)))
           `(,(wfc-error o (format #f "valued ~a must be used in variable assignment" name))))
          (else '()))))
