...
(define-templates reply (lambda (o)
                           (if (is-a? o <void>) ""
                               (string-append " " (with-output-to-string (cut x:non-void-reply o)))))) ;; MORTAL SIN HERE!!?(define-templates capitalize-model-name (compose (cut string-upcase <> 0 1) symbol->string om:name (lambda (o) (parent <model> o))))
(define-templates decapitalize-model-name (compose (cut string-downcase <> 0 1) symbol->string om:name (lambda (o) (parent <model> o))))
(define-templates upcase-model-name (compose string-upcase (->join "_") om:scope+name (lambda (o) (parent <model> o))))
...
(define-templates port-release (lambda (o) (if (om:blocking-compound? (parent <model> o)) o "")))
...
(define-templates enum-definer (lambda (o) (append (om:enums o) (filter (is? <enum>) (om:globals o)))))
