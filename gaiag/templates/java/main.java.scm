...
#(map
    ...
    m.#port .#direction .#event  = (#formals) -> {#(string-if (is-a? type-type <void>) #{log_#direction("#port .", "#event ", e);#}#{return log_valued("#port .", "#event ", e, "#port .#reply-name _", #(if (or (null? reply-scope) (om:outer-scope? model reply-scope)) 'DznGlobal reply-scope).#reply-name .class);#})};
#}) (filter (negate (om:dir-matches? port))
       (om:events port)))) (om:ports model))
#(map (init-port #{
    m.#name .in.self = c;
    m.#name .in.name = "<internal>";
    e.put("#name .<flush>", () -> {System.err.println("#name .<flush>"); m.runtime.flush (m.#name .in.self);});
