...
(define-method (initialize (o <dezyne:#.scope_model >) args)
  ...
  (let-keywords
   args ##f ((locator ##f)
            ...
            (parent ##f)
            #((->join "\n            ")
 (map (init-bind model #{(#port .#edir  (make <dezyne:#((om:scope-name) interface) .#edir >))#})
      (filter bind-port? (filter (negate injected-binding?) ((compose .elements .bindings) model))))))#
(map (init-instance #{#'()
  (set! (.#name  o) (make <dezyne:#((om:scope-name) component) > :locator (.locator o) :parent o :name '#name))#})
