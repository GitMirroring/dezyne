      ...
      (lambda (f)
        (let* ((signature (.signature f))
               ...
               (p-types (if (pair? parameters)
                            (->string (list "." (csp-comma-list (map (lambda (x) (if (is-a? x <enum>) (->string (.name model) '_ (.name x)) (.name x))) (map (compose (om:type model) .type) ((compose .elements .parameters .signature) f))))  "\n"))
                            ""))
               (type (.type signature))
