 ...
 #(map
   (lambda (port)
     (map (define-on model port #{m.#port .#direction .#event  = [&] (#formals) {#(string-if (eq? return-type 'void) #{call_#direction("#port .", "#event ", e);#}
                                                                                                                 #{return call_valued<#((c++:scope-join #f) reply-scope)::#reply-name ::type>("#port .", "#event ", e, #((c++:scope-join #f) (if (null? reply-scope) '() (drop-right reply-scope 1)))::to_#((om:scope-join #f) reply-scope)_#reply-name , static_cast<const char*(*)(#((c++:scope-join #f) reply-scope)::#reply-name ::type)>(#((c++:scope-join #f) (if (null? reply-scope) '() (drop-right reply-scope 1)))::to_string));#})};
