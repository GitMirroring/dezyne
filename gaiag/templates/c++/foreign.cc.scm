...
#name(#(if (not (eq? expression *unspecified*)) expression))#}) (om:variables model))
  (map (lambda (port) (list (.name port) "{" (if (.injected port) (list "dzn_locator.get<" ((c++:scope-name) (.type port)) ">()") (list "{" (if (eq? (.direction port) 'requires) "{\"\",0,0},") "{\"" (.name port) "\",this,&dzn_meta}" (if (eq? (.direction port) 'provides) ",{\"\",0,0}") "}")) "}")) (om:ports model))))
  {
