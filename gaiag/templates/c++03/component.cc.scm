...
: dzn_meta("","#.model",reinterpret_cast<const component*>(this),0)
...
{
#((->join "\n") (map (lambda (port) (list "dzn_meta.ports_connected.push_back(boost::function<void()>(boost::bind(&" (.type port) "::check_bindings,&" (.name port) ")));"))
                      (om:ports model)))
...
#
   (map
    (lambda (port)
      (map (define-on model port #{#port .#direction .#event  = boost::bind(&#(string-if (eq? return-type 'void) "call_in<" #{rcall_in<#(*scope* reply-scope) ::#reply-name ::type, #})#.model ,#interface #comma #((->join ",") formal-types)>,this,boost::function<#return-type(#((->join ",") formal-types))>(boost::bind(&#.model ::#port _#event ,this#comma #((->join ",") (map (lambda (x) (->string '_ (+ 1 x))) (iota (length formal-list))))))#comma #((->join ",") (map (lambda (x) (->string '_ (+ 1 x))) (iota (length formal-list)))),boost::make_tuple(&#port , "#event ", "return"));
#}) (filter om:in? (om:events port))))
    (filter om:provides? (om:ports model)))#
(map
    (lambda (port)
      (map (define-on model port #{
#port .#direction .#event  = boost::bind(&call_out<#.model , #interface #comma #((->join ",") formal-types)>, this, boost::function<#return-type(#((->join ",") formal-types))>(boost::bind(&#.model ::#port _#event , this #comma #((->join ",") (map (lambda (x) (->string '_ (+ 1 x))) (iota (length formal-list))))))#comma #((->join ",") (map (lambda (x) (->string '_ (+ 1 x))) (iota (length formal-list)))), boost::make_tuple(&#port , "#event ", "return"));
