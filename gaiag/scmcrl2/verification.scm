  ...
  (let* ((cwd (getcwd))
    ...
    (receive (job port)
      ...
      (let ((output (read-string port))
        ...
        (chdir cwd)
        (if (not (gdzn:command-line:get 'json))
            (let* ((output (string-trim-right output))
                   (trace (if (string-null? trace) trace (string-append trace "\n") )))
              (if (string-length output) (stderr "~a\n" output))
...
(define (assert-fail-compliance dir file-name model-type model-name assert spec-trace-file impl-trace-file impl-trace verbose?)
  (let* ((cwd (getcwd))
         (foo (chdir dir))
         (commands (list
                    ;;(display trace) ;; FIXME
    ...
    (receive (job port)
        (apply pipeline #f commands)
      (let ((output(read-string port))
            (trace (string-trim-right impl-trace)))
        (chdir cwd)
        (if (not (gdzn:command-line:get 'json))
            (let* ((output (string-trim-right output))
                   (trace (if (string-null? trace) trace (string-append trace "\n"))))
...
(define (check-compliance string dir file-name model-name verbose?)
  (let ((match? (regexp-exec (make-regexp "Saved trace to file (.*)\nThe LTS in (.*) is not included in the LTS .*") string))
    ...
    (if match? (let* ((trace (make-trace-file (match:substring match? 1) assert dir file-name model-name))
                 ...
                 (assert-fail-compliance dir file-name model-type model-name assert spec-trace-file impl-trace-file impl-trace verbose?))
        (assert-ok model-type model-name assert verbose?))))
