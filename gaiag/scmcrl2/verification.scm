...
(define (deterministic-labels component)
  (define (compose-triggers channel dir triggers)
    (map (lambda (t) (string-append (symbol->string (.port.name t)) channel "(" (symbol->string ((compose verify:scope-name .type.name .port) t)) "'action" "("
                                    (symbol->string ((compose verify:scope-name .type.name .port) t)) dir (symbol->string (.event.name t)) ")" ")")) triggers))
  (string-join (append (compose-triggers "in" "'in'" (ast:provided-in-triggers component))
                       (compose-triggers "qout" "'out'" (ast:required-out-triggers component))) ","))
...
(define ((mcrl2:verify-interface-deadlock-livelock model) dir file-name ast verbose? all?)
  (let* ((model-name ((compose ->string verify:scope-name) model))
         ...
         (commands `(,(cut x:interface-init model)
                     ...
                     ("ltsconvert" "-edpweak-bisim" "--in=lts" "--out=aut")
                     ("sed" "-e" "s,\"declarative_illegal\",\"dillegal\",g")
                     ("lts" "-s" "-d" "-t" ,taus "-l" "/dev/stdin")
                     ))
...
(define ((mcrl2:verify-component-deterministic-illegal-deadlock-livelock-refinement model) dir file-name ast verbose? all?)
  (define (verify-component-refinement lts info model-name ast)
    (let* ((foo (assert-start 'component model-name 'compliance verbose?))
           ...
           (commands `(,(cut x:provides-init ast)
                       ...
                       ("sed" "-e" "s,\"declarative_illegal\",\"dillegal\",g")
                       ("lts" "-f" "/dev/stdin")
                       ("ltscompare" "-v" "-c" "-pweak-failures" ,@taus "--in1=aut" "--in2=aut" "component" "/dev/stdin")))
           (error (receive (job ports)
                      (apply pipeline+ #f commands)
                    (set-port-encoding! (car ports) "ISO-8859-1")
                    (let ((error (read-string (cadr ports))))
  ...
  (let* ((model-name ((compose ->string verify:scope-name) model))
         ...
         (commands `(,(cut x:component-init ast)
                     ...
                     ("ltsconvert" "-edpweak-bisim" "--in=lts" "--out=aut")
                     ("sed" "-e" "s,\"declarative_illegal\",\"dillegal\",g")
                     ("lts" "-s"  "--nondet" ,deterministic "-i" "illegal" "-d" ,@taus "-l" "-f" "/dev/stdin")))
         (result (receive (job ports)
                    (apply pipeline+ #f commands)
                   (set-port-encoding! (car ports) "ISO-8859-1")
                   (let ((result (read-string (car ports)))
