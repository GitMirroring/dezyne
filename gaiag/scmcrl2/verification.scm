...
(define (assert-fail dir file-name model-type model-name assert trace message interface-trace)
  ...
  (let* ((interface-trace-file (and interface-trace (canonicalize-path "interface-trace.txt")))
         ...
         (last (and (pair? trace-list) (last trace-list)))
         (message (cond
                    ...
                    (else message)))
         (trace (if (member last '("range_error" "type_error" "double_reply_error" "no_reply_error")) (string-join (drop-right trace-list 1) "\n")
...
(define (check-deterministic string dir file-name model-type model-name verbose?)
  (let ((match? (regexp-exec (make-regexp "Nondeterministic state found and saved to '([^'\n]*)'.*") string))
    ...
    (if match? (let ((trace (mcrl2-trace-file->dzn-trace (match:substring match? 1)))
                     (message (format #f "Component ~a is non-deterministic due to overlapping guards" model-name)))
...
(define (check-compliance string dir file-name model-type model-name verbose?)
  (let ((match? (regexp-exec (make-regexp "Saved trace to file (.*)\nThe LTS in (.*) is not included in the LTS .*") string))
    ...
    (if match? (let* ((trace (mcrl2-trace-file->dzn-trace (match:substring match? 1)))
                      (trace (string-trim-right trace))
                      (trace (if (string-null? trace) trace (string-append trace "\n")))
                      (component-accepts (cond ((string-match "The acceptance of the left process is empty." string) #f)
                      ...
                      (interface-accepts (cond ((string-match "The process at the right has no acceptance sets" string) #f)
