  ...
  (let* ((model (or (ast:interface ast) (ast:component ast)))
    ...
    (->string
     (match src
       ...
       (('return vars) (let* ((transition-end (if component? "transition_end -> "))
                              (channel-return (if provided-on? (list channel ".return -> ")))
                              (vars (comma-join vars))
                              (end (if (not inevitable-optional?) (list channel-return transition-end))))
