    ...
    (->string
     (match src
       ...
       (('assign vars expressions) 
        (let ((vars (comma-join vars))
              (expressions (comma-join (map (lambda (x) (csp-transform ast x)) expressions))))
          (list "assign(\\(" vars ") @ (" expressions "))")))
       (('if vars expression then else) 
        (let ((expression (csp-transform ast expression))
          ...
          (list "\\P,(" vars ") @ ifthenelse(" expression "," then "," else ")(P,(" vars "))")))
