    ...
    (match src
             ...
             (let* ((statement (car statements))
               ...
               (if (>1 (length statements))
                   (if (equal? transformed '(action illegal))
                       transformed
                       (list 'semi transformed (loop (cdr statements) locals)))
                   transformed)))))
	 ...
	 (if (prefix-illegal? stat)
      ...
      (('assign var ('value type field))
       (list 'assign variables (cons (map (assignment var field) members)
                                     (map (assignment var field) locals))))
      (('assign var exp)
       (list 'assign variables (cons (map (assignment var exp) members)
  ...
  (let* ((model (or (ast:interface ast) (ast:component ast)))
    ...
    (->string
     (match src
       ...
       (('assign vars (expressions locals ...))
        (let* ((var-members (comma-join (map (lambda (x) (csp-transform ast x)) (car vars))))
               (var-locals (comma-join (map (lambda (x) (csp-transform ast x)) (cdr vars))))
