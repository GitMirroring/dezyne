...
(define* (ast-transform-return ast src :optional (top? #t))
  (let* ((model (or (ast:interface ast) (ast:component ast)))
    ...
    (match src
	 ...
	 (if (>1 (length result))
	     (list 'on events (list 'compound result (list 'return)) `(the-end ,variables))
  ...
  (let* ((model (or (ast:interface ast) (ast:component ast)))
    ...
    (match src
	 ...
	 (if (null? statements)
		 ...
		 (if (equal? (car statements) '(action illegal))
		     (car statements)
	 ...
	 (if (prefix-illegal? stat)
      ...
      (('variable type var expr)
       (list 'assign (list 'variable type var expr)))
      (('assign var ('value type field))
       (list 'assign variables (map (lambda (x) (if (eq? x var ) field x)) variables)))
      (('assign var exp)
       (list 'assign variables (map (lambda (x) (if (eq? x var ) exp x)) variables)))
      (('if pred then)
       (list 'if variables (list 'expression (if (prefix-illegal? then)
                                                 (list 'and 'IG pred)
                                                 pred))
             (ast-transform- ast then return) '()))
      (('if expr then else)
    ...
    (->string
     (match src
       ...
       (('assign ('variable type var action))
        (list "\\P',V' @ " (cadr action) "!" (caddr action) " -> " (cadr action) "?" var " -> P'(V')"))
       ...
       (('if vars expression then else)
        (let ((expression (csp-transform ast expression))
          ...
          (list "\\P,(" vars ") @ ifthenelse(" expression ",\n" then ",\n" else "\n)(P,(" vars "))")))
