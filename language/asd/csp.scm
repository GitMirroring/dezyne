...
(define ((valued-action? port?) src)
  (match src
    (('variable type var (? (a-is? <action>))) #t)
    (('variable type var ('value (? port?) action)) #t)
    (('assign var (? (a-is? <action>))) #t)
    (_ #f)))
...
;;       (('vector expressions ...) (cons 'vector (map (lambda (exp) (csp-transform ast exp)) expressions )))
       ...
       (('context-active (context var ('expression (and (? (a-is? <action>)) (get! action)))) stat)
        (let ((stat (csp-transform ast stat inevitable-optional? channel provided-on?))
       ...
       (('assign-active (context var (and (? (a-is? <action>)) (get! action))) expressions)
        (let* ((trigger (.trigger (action)))
               (action (list "sendrecv_(" (.port trigger) "," (.event trigger) ")")))
