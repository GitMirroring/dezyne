...
(define* (ast-transform ast src :optional (return #t))
  (let* ((model (or (ast:interface ast) (ast:component ast)))
    ...
    (match src
	 ...
	 (list 'if variables (list 'expression pred) (ast-transform ast then return) (ast-transform ast else return))))
      (_ src))))
...
(define* (csp-transform ast src :optional (inevitable-optional? #f) (channel #f) (provided-on? #t))
  (let* ((model (or (ast:interface ast) (ast:component ast)))
    ...
    (match src
      ...
      (('on events stat ...) 
       (let* ((inevitable-optional? (or (member 'inevitable events)
					(member 'optional events)))
	      (ig? (and (pair? stat) (eq? (car stat) 'IG)))
	      (channel (if (ast:interface? model) model-name (ast:port-name (car events))))
	      (provided-on? (or (ast:interface? model) ((provides-event? model) (car events)))))
	 (if ig? (set! stat (cdr stat)))
	 ((->join " -> ") (list (->string (list (if ig? 
						    (if ((provides-event? model) (car events)) "IIG & " "IG & ")
						    "") channel "?x:{" (comma-join (map ast:event-name events)) "}")) (map (lambda (x) (csp-transform ast x inevitable-optional? channel provided-on?)) stat)))))
      (('return vars) 
       (->string (list "(\\(" (comma-join vars) ") @ " 
		       (if inevitable-optional? "" 
			   (->string (list (if provided-on? (list channel ".return -> ") "")  (list (if (ast:component? model) "transition_end -> " "")))))
		       model-name "_" behaviour "_" "((" (comma-join vars) ")),(" (comma-join vars) "))")))
      (('action 'illegal) "illegal -> STOP")
      (('action event) 
       (let ((channel (if (ast:interface? model) model-name (ast:port-name event))))
	 (->string (list "\\P,V @ " channel "!" (ast:event-name event)
			 (if ((requires-event? model) event) (list " -> " channel ".return") "") " -> P(V)"))))
      (('assign vars exprs) (->string (list "assign(\\(" (comma-join vars) ") @ (" (comma-join (map (lambda (x) (csp-transform ast x)) exprs)) "))")))
      (('if vars expression then else) (->string (list "\\P,(" (comma-join vars) ") @ ifthenelse(" (csp-transform ast expression) "," (csp-transform ast then inevitable-optional? channel provided-on?) "," (csp-transform ast else inevitable-optional? channel provided-on?) ")(P,(" (comma-join vars) "))")))
      (('expression e) (csp-transform ast e))
      (('! e) (->string (list "not (" (csp-transform ast e) ")")))
      (('or lhs rhs) (->string (list "(" (csp-transform ast lhs) " or " (csp-transform ast rhs) ")")))
      (('and lhs rhs) (->string (list (csp-transform ast lhs) " and " (csp-transform ast rhs))))
      (('== lhs rhs) (->string (list (csp-transform ast lhs) " == " (csp-transform ast rhs))))
      (('!= lhs rhs) (->string (list (csp-transform ast lhs) " != " (csp-transform ast rhs))))
      (('semi stat1 stat2) (->string (list "semi(" (csp-transform ast stat1 inevitable-optional? channel provided-on?) "," (csp-transform ast stat2 inevitable-optional? channel provided-on?) ")")))
      ('() "\\P,V @ P(V)")
      (_ src))))
