...
(define (is-op? c) (string-contains "+-*/o@" (string c)))
  ...
  (let ((c (peek-char port)))
    (cond
     ...
     (else
      (read-char port)
      (next-token port)))))
(define (make-asd-tokenizer port) (lambda () (next-token port)))
(define-public *tree* "")
(define (make-parser)
  (lalr-parser
   (o number NumericLiteral @ (left: + -) (left: * /))
   (program (exp) : $1
...
(define (comp src e)
  (format #t "MATCHING:~a\n" src)
  (match src
    ((x) `(apply (primitive list) (const 4) (const x)))
    ((number x) `(const ,x))
    ((NumericLiteral x) `(const ,x))
    ((list x y) (begin (format #t "LIST:~a:~a:~a\n" src e x) `(apply (primitive list) (const ,x) ,(comp y e))))
