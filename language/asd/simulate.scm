...
(define (next-todo module events)
...
(define* (simulate module :optional 
  ...
  (let loop ((state-todo (next-todo module events)))
    (if (null? state-todo)
        '()
        (let ((state (car state-todo))
          ...
          (if (or (not state)
              ...
              '()
              (let ((event (car todo)))
                (let loop-transition ((state state) (event event))
                  (if (not state)
                      (loop (next-todo module events)))
                  (let ((next (process-event ast state event)))
                    (if (not next)
                        (loop (next-todo module events))
                        (let ((done (seen next ast)))
                          (receive (done todo) 
                              (partition (lambda (x) (member x done equal?)) events)
                            (if (null? todo)
                                (loop (next-todo module events))
                                (loop-transition next (car todo))))))))))))))
