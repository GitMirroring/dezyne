...
struct #.interface 
{
 #(->string (map declare-enum ((compose ast:type-list ast:interface) ast)))
};
...
#}  (filter ast:in? ((compose ast:event-list ast:interface) ast)))
...
# (map-events
#{  virtual void #.event () = 0;
#} (filter ast:out? ((compose ast:event-list ast:interface) ast)))
