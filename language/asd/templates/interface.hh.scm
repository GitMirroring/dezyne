...
struct #.interface 
{
 #(->string (map declare-enum ((compose ast:types ast:interface) ast)))
};
...
#}  (filter ast:in? ((compose ast:events ast:interface) ast)))
...
# (map-events
#{  virtual void #.event () = 0;
#} (filter ast:out? ((compose ast:events ast:interface) ast)))
