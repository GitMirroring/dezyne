...
#} ((compose ast:ports ast:system) ast))
  ...
  struct #.component 
  {
    #(->string (map declare-enum (ast:types (ast:behaviour (ast:system ast)))))
  };
...
#} ((compose ast:instances ast:system) ast))

# (string-if (ast:behaviour (ast:system ast))
#{
    State* m_State;
    State& getState();
  public:
...
# (map-variables
#{      #.state-type  #.variable ;
#} (ast:variables (ast:behaviour (ast:system ast))))
      Predicates()
      {
# (map-variables
#{        #.variable  = #.value ;
...
#} ((compose ast:binds ast:system) ast))
  }
