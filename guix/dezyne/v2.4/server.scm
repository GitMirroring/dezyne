...
(define-module (dezyne v2.4 server)
...
(define-public dezyne-server-10
  (let ((version "10"))
    (package
      (name "dezyne-server")
      (version version)
      ...
      (native-inputs `(("dezyne-services" ,dezyne-services-2.4)
                       ("guile" ,guile-2.2)
...
(define-public dzn-daemon-tarball-10
  (package
    (name "dzn-daemon-tarball")
    (version "10")
    (source #f)
    (native-inputs `(("dezyne-server" ,dezyne-server-10)))
    (build-system gnu-build-system)
    (arguments
     `(#:tests? #f
       #:modules ((srfi srfi-1)
...
(define-public node-dzn-daemon-10
  (package
    (name "node-dzn-daemon")
    (version "0.0.10")
    (source #f)
    (native-inputs `(("dzn-daemon-tarball" ,dzn-daemon-tarball-10)))
