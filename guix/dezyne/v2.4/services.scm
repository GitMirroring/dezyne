...
(define-module (dezyne v2.4 services)
  ...
  #:use-module (dezyne v2.4 extra)
  #:use-module (dezyne config)
  #:use-module (dezyne extra))

(define-public dezyne-source-2.4
  (origin
    (method git-fetch)
    (uri (git-reference
          (url (string-append git.oban/blessed "/development.git"))
          (commit "2.4.1")))
    (sha256 (base32 "1dxl29gr1c7zvdkmipzmxna9i38pnm3c9rhy212r1pxg7wx4nwdc"))))

(define-public dezyne-services-2.4
  (package
    (name "dezyne-services-2.4")
    (version "2.4.1")
    (source dezyne-source-2.4)
    (propagated-inputs `(("asd-converter" ,asd-converter-0.1.3)
                         ("bash" ,bash)
...
(define-public dzn-client-tarball-2.4
  (package
...
;; (define-public node-dzn-2.4
;;   (package
;;     (name "node-dzn")
;;     (version "2.4.1")
;;     (source #f)
;;     (native-inputs `(("dzn-client-tarball" ,dzn-client-tarball-2.4)))
;;     (propagated-inputs `(("bash" ,bash)
;;                          ("node-dzn-daemon" ,node-dzn-daemon-11) ; build with old daemon?
;;                          ("node" ,node6)
;;                          ("node-snapshot" ,node-snapshot)))
;;     (build-system gnu-build-system)
;;     (arguments
