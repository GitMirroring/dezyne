...
(define-module (main)
  ...
  #:use-module (dzn pump)
  #:use-module (sync_out_reply_binding)
...
(define (main . args)
  (let* ((print-illegal (lambda () (stderr "illegal\n") (exit 0)))
         ...
         (sut (make <sync_out_reply_binding> #:locator locator #:name "sut"))
    ...
    (set! (.hello (.in (.w sut))) (lambda _
                                    ...
                                    (action sut .w .out .world)
                                    (dzn:trace-out log (.w sut) "return")))
    (set! (.hello_void (.in (.w sut))) (lambda _
                                         (dzn:trace log (.w sut) "hello_void")
                                         (action sut .w .out .world_void)
                                         (dzn:trace-out log (.w sut) "return")))
