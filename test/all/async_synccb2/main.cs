...
public static class main
{
	...
	using(dzn.pump pump = new dzn.pump()) {
	    async_synccb2 sut = new async_synccb2(loc.set(rt).set(pump));
	    ...
	    sut.p.outport.cb = () => {System.Console.Error.WriteLine("sut.p.cb -> <external>.p.cb [" + t + "]");};
            sut.r.inport.e = () => {System.Console.Error.WriteLine("sut.r.e -> <external>.r.e [" + t + "]");
                                    System.Console.Error.WriteLine("sut.r.return -> <external>.r.return");};
            sut.r.inport.c = () => {System.Console.Error.WriteLine("sut.r.c -> <external>.r.c [" + t + "]");
                                    System.Console.Error.WriteLine("sut.r.return -> <external>.r.return");};
		...
		pump.shell (() => {sut.p.inport.e ();sut.r.outport.cb1 ();sut.r.outport.cb2 ();sut.p.inport.c ();});
	    }
            else if (trace == "p.e\nr.e\nr.return\np.return\nr.cb1\nr.cb2\np.cb")
            {
                pump.shell (() => {sut.p.inport.e ();sut.r.outport.cb1 ();sut.r.outport.cb2 ();});
            }
	    else if (trace == "p.c\nr.c\nr.return\np.return") {
		pump.shell (() => {sut.p.inport.c ();});
	    }
