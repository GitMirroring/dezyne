...
;;USAGE: in dzn.git root: make check TESTS=test/gaiag/language

(define-module (test gaiag language)
  #:use-module (gaiag commands language)
...
(define (assert-completion-options options dzn-file)
  (let* ((output (with-output-to-string (cut main (append (list "gaiag/commands/language.scm") options (list dzn-file)))))
...
(define (assert-completion dzn-file)
  (let ((output (with-output-to-string (cut main (list "gaiag/commands/language.scm" dzn-file))))
        (baseline (call-with-input-file (string-append dzn-file ".baseline") read-string)))
