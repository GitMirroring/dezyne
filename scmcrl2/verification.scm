...
(define ((mcrl2:verify-component-deterministic-illegal-deadlock-livelock-refinement model) dir dzn-file-name ast verbose? all?)
  (let* ((model-name ((compose ->string verify:scope-name) model))
         ...
         (refinement (do-refinement lts model-name makreel model verbose?)))
    (reduce-or all?
               (list (cut check-deterministic (get-trace "deterministic" result) info dir dzn-file-name 'component model-name verbose?)
                     ...
                     (cut check-compliance refinement info dir dzn-file-name 'component model-name verbose?)))))
...
(define (check-compliance error info dir dzn-file-name model-type model-name verbose?)
  (let* ((trace (and (string-contains error "is not included in the LTS")))
