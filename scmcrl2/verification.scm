...
(define %dzn (if (not (string-suffix? "mingw32" %host-type)) %dzn "dzn.cmd"))

(define %shell (if (not (string-suffix? "mingw32" %host-type)) '("sh" "-c") '("cmd" "/c")))
...
(define ((mcrl2:verify-interface-deadlock-livelock model) dir dzn-file-name ast verbose? all?)
  (let* ((model-name ((compose ->string verify:scope-name) model))
         ...
         (commands `(,(cut format #t "~a\n~a\n" intf (x:interface-init model))
                     ...
                     ("lpsconstelm" "-st")
                     ("lpsparelm")
                     ,(if %lps2lts-stdout? '("lps2lts" "--cached" "--out=aut" "-")
                          '("bash" "-c" "set -e; rm -f lts0.aut; lps2lts --cached --out=aut /dev/stdin lts0.aut; cat lts0.aut; rm -f lts0.aut"))
...
(define (do-refinement lts model-name makreel model verbose?)
  (let* ((foo (assert-start 'component model-name 'compliance verbose?))
         ...
         (commands `(,(cut format #t "~a\n~a\n" makreel (x:provides-init model))
                     ...
                     ("lpsparelm")
                     ,(if %lps2lts-stdout? '("lps2lts" "--cached" "--out=aut" "-")
                          '("bash" "-c" "set -e; rm -f lts2.aut; lps2lts --cached --out=aut /dev/stdin lts2.aut; cat lts2.aut; rm -f lts2.aut"))
                     ("ltsconvert" "-edpweak-bisim" "--in=aut" "--out=aut")
                     ("sed" "-e" "s,\"declarative_illegal\",\"dillegal\",g")
                     (,%dzn "lts" "--failures" "-")))
         (interface (apply pipeline->string commands)))
    (if %ltscompare-structured-output?
        (let ((commands `(,(cut format #t "~a\n~a" lts interface)
                          (,@%shell ,(string-append "ltscompare --quiet --counter-example --structured-output -pweak-failures" taus " --in1=aut --in2=aut - -")))))
          (when (> (gdzn:debugity) 0)
            (format (current-error-port) "commands: ~s\n" (apply string-join (cdr commands))))
          (receive (output status)
            ...
            (let* ((lines (and output (string-split output #\newline)))
                   ...
                   (trace (and trace (string-trim-right trace)))
                   (trace (and trace (rename-lts-actions trace)))
                   ...
                   (component-accepts (and component-accepts (substring component-accepts (1+ (string-index component-accepts #\:)))))
                   ...
                   (interface-accepts (and interface-accepts (substring interface-accepts (1+ (string-index interface-accepts #\:)))))
                   (interface-accepts (and component-accepts (rename-lts-actions interface-accepts))))
              (when (> (gdzn:debugity) 0)
        ...
        ((if (> (gdzn:debugity) 0) (lambda (proc) (proc "out")) call-with-temporary-directory)
         (lambda (tmpdir)
           (with-directory-excursion tmpdir
             (let* ((chatter (if %ltscompare-stdin? (let ((commands `(,(cut format #t "~a\n~a" lts interface)
                                                                      (,@%shell ,(string-append "ltscompare --verbose --counter-example -pweak-failures" taus " --in1=aut --in2=aut - - 2>&1")))))
                                                      (apply pipeline->string commands))
                                 (begin
                                   (with-output-to-file "component" (cut display lts))
                                   (with-output-to-file "interface" (cut display interface))
                                   (let ((commands `((,@%shell ,(string-append "ltscompare --verbose --counter-example -pweak-failures" taus " --in1=aut --in2=aut component interface 2>&1")))))
                                     (when (> (gdzn:debugity) 0)
                                       (format (current-error-port) "commands: ~s\n" (apply string-join commands)))
                                     (apply pipeline->string commands)))))
                    (trace (and (string-contains chatter "is not included in the LTS")))
                    (trace (and trace (pipeline->string '("tracepp" "counter_example_weak_failures_refinement.trc"))))
                    (trace (string-trim-right trace))
                    (trace (and trace (rename-lts-actions trace)))
                    (trace (if (string-null? trace) trace (string-append trace "\n")))
                    (component-accepts (cond ((string-match "The acceptance of the left process is empty." chatter) #f)
                                             ((string-match "A stable acceptance set of the left process is:\n([^\n]*)\n" chatter) => (cut match:substring <> 1))
                                             ((string-match "is not included in the LTS in" chatter) "")
                                             (else (stderr "other string:~s\n" chatter))))
                    (component-accepts (and component-accepts (rename-lts-actions component-accepts)))
                    (interface-accepts (cond ((string-match "The process at the right has no acceptance sets" chatter) #f)
...
(define ((mcrl2:verify-component-deterministic-illegal-deadlock-livelock-refinement model) dir dzn-file-name ast verbose? all?)
  (let* ((model-name ((compose ->string verify:scope-name) model))
         ...
         (commands `(,(cut format #t "~a\n~a\n" makreel (x:component-init model))
                     ...
                     ("lpsparelm")
                     ,(if %lps2lts-stdout? '("lps2lts" "--cached" "--out=aut" "-")
                          '("bash" "-c" "set -e; rm -f lts3.aut; lps2lts --cached --out=aut /dev/stdin lts3.aut; cat lts3.aut; rm -f lts3.aut"))
                     ("ltsconvert" "-edpweak-bisim" "--in=aut" "--out=aut")
                     ("sed" "-e" "s,\"declarative_illegal\",\"dillegal\",g")
